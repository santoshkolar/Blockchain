...
describe('Starting tests for g-santhu-angular', function() {
  ...
  it('website title should be g-santhu-angular', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('g-santhu-angular');
    })
  });

  it('network-name should be gowdanar-funds@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('gowdanar-funds@0.0.1.bna');
    });
  });

  it('navbar-brand should be g-santhu-angular',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('g-santhu-angular');
    });
  });
